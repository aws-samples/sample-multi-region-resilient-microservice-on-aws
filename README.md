...
| CKV_AWS_2 / CKV_AWS_103 | The ALB listener uses HTTP instead of HTTPS with TLS v1.2.                                                            | The ALB is internal (Scheme: internal) with no public exposure. Canary health checks use HTTP. In production, configure an ACM certificate and HTTPS listener.                                                                                                                                                        |
| CKV_AWS_111 | IAM policies allow write access without resource constraints.                                                                    | ARC Region Switch role requires broad permissions for cross-service failover orchestration. `cloudwatch:PutMetricData` does not support resource-level permissions. Aurora Lambda roles are mitigated with `aws:ResourceAccount` conditions.                                                                           |
| CKV_AWS_51  | ECR image tags are mutable.                                                                                                      | The build system reuses git commit SHA as the image tag across deployments. Immutable tags would prevent redeployment of the same commit. In production, use unique tags per build.                                                                                                                                   |
| RESTRICTED_INCOMING_TRAFFIC | Windows security group allows ingress on port 3389.                                                                 | False positive. RDP ingress is restricted to the VPC CIDR (`10.1.0.0/16`), not `0.0.0.0/0`. The rule flags any SG with port 3389 regardless of CIDR scope.                                                                                                                                                          |
...
| CKV_AWS_116 | Lambda functions do not have a Dead Letter Queue configured.                                                                     | These are infrastructure-utility Lambdas (secret rotation, write forwarding, DNS status) invoked synchronously by CloudFormation custom resources or EventBridge rules, not asynchronous event-driven functions. DLQs do not apply to synchronous invocations.                                                         |
| CKV_AWS_117 | Lambda functions are not configured inside a VPC.                                                                                | The ARC DNS status Lambda calls only AWS API endpoints (ARC, CloudWatch) and does not access VPC resources. Placing it in a VPC would require NAT Gateway for no security benefit.                                                                                                                                    |
| CKV_OPENAPI_4 / CKV_OPENAPI_5 / CKV_OPENAPI_21 | OpenAPI specs missing global security definitions and array max items.                                  | These are internal service-to-service API specs used for client code generation. The services are not publicly exposed; they run behind an internal ALB within a VPC.                                                                                                                                                 |
| CKV_AWS_31  | ElastiCache Replication Group does not have an auth token.                                                                       | Adding an auth token requires application code changes to pass Redis credentials. Transit encryption (TLS) and at-rest encryption are enabled. The Redis cluster is in a private subnet accessible only from ECS task security groups.                                                                                 |
