  ...
  steps: [
    ...
    },
    {
      ...
      run: [
        ...
        '# baseInfra owns the interface VPC endpoints that the database stacks\'',
        '# custom-resource Lambdas and Aurora ENIs depend on. Deleting it while a',
        '# database stack is still present severs those paths, so the database',
        '# stack then sits in DELETE_IN_PROGRESS until CloudFormation times out an',
        '# hour later and fails, leaving Aurora running and billing. Retry the',
        '# database stacks here first and only touch the VPC once they are gone.',
        'for region in ${{ env.AWS_REGION }} ${{ env.STANDBY_REGION }}; do',
        '  for stack in catalog-db-stack${{ env.ENV }} carts-db-stack${{ env.ENV }} orders-dsql-stack${{ env.ENV }}; do',
        '    status=$(aws cloudformation describe-stacks --stack-name "$stack" --region "$region" --query "Stacks[0].StackStatus" --output text 2>/dev/null || echo ABSENT)',
        '    if [ "$status" != "ABSENT" ]; then',
        '      echo "$region: $stack still present ($status), deleting before the VPC"',
        ...
        '  done',
        'done',
        '# Any database stack still present is now wedged. Retain the custom',
        '# resource so the delete can reach the billable Aurora cluster instead of',
        '# aborting on a callback that can no longer be answered.',
        'for region in ${{ env.AWS_REGION }} ${{ env.STANDBY_REGION }}; do',
        '  status=$(aws cloudformation describe-stacks --stack-name catalog-db-stack${{ env.ENV }} --region "$region" --query "Stacks[0].StackStatus" --output text 2>/dev/null || echo ABSENT)',
        '  if [ "$status" = "DELETE_FAILED" ]; then',
        '    echo "$region: catalog-db-stack is DELETE_FAILED, retrying without the custom resource"',
        '    aws cloudformation delete-stack --stack-name catalog-db-stack${{ env.ENV }} --region "$region" --retain-resources InvokeCustomLambda || true',
        '    aws cloudformation wait stack-delete-complete --stack-name catalog-db-stack${{ env.ENV }} --region "$region" || true',
        '  fi',
        'done',
        'for region in ${{ env.AWS_REGION }} ${{ env.STANDBY_REGION }}; do',
