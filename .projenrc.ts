  ...
  steps: [
    ...
    },
    {
      ...
      run: [
        ...
        '    --query "StackResources[?LogicalResourceId==\'EcsCluster\']|[0].PhysicalResourceId" --output text)',
        '  CLUSTER_NAME=$(basename "$CLUSTER_ARN")',
        '  SERVICES=""',
        '  for lid in CartsService OrdersService AssetsService CatalogService CheckoutService UiService; do',
        '    SVC=$(aws cloudformation describe-stack-resources \\',
        '      --stack-name apps${{ env.ENV }} --region "$region" \\',
        '      --query "StackResources[?LogicalResourceId==\'$lid\']|[0].PhysicalResourceId" --output text 2>/dev/null)',
        '    [ -n "$SVC" ] && [ "$SVC" != "None" ] && SERVICES="$SERVICES $(basename $SVC)"',
        '  done',
